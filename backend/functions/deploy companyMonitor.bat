...
:PRO_DEPLOY
CD companyMonitor
gcloud functions deploy companyMonitor --runtime python311 --trigger-event providers/cloud.firestore/eventTypes/document.update --trigger-resource "projects/flameoapp-pyme/databases/(default)/documents/companies/{companyID}" --project flameoapp-pyme --region europe-central2 --entry-point handle_update --env-vars-file .env.yaml
GOTO END
